...
// session matching projectPath. Session directories are located by finding
// workspace.yaml files anywhere under the state directory (not just as
// immediate children), since Copilot CLI has changed the nesting depth of
// session directories across versions (e.g. flat <id>/workspace.yaml vs.
// grouped <group>/<id>/workspace.yaml).
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	_ = filepath.WalkDir(stateDir, func(path string, d fs.DirEntry, err error) error {
