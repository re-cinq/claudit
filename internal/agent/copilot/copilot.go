...
// It walks recursively (rather than assuming a fixed one-level directory layout)
// so it keeps working if newer Copilot CLI versions nest session data differently,
// and it accepts a match on either the recorded cwd or git_root, since some
// versions only populate one of the two.
