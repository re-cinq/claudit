...
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// Match against either the recorded working directory or the git
		// repository root. Newer Copilot CLI releases have been observed to
		// populate only one of these two fields depending on how the CLI was
		// invoked, so both are treated as valid matches.
