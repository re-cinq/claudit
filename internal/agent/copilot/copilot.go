...
// scanForRecentSession recursively scans Copilot's session state directory
// for recent session directories. Newer Copilot CLI releases may nest
// session directories more deeply (e.g. grouped under a workspace-specific
// parent), so a single-level directory listing is not sufficient; instead
// we walk the whole tree looking for workspace.yaml files. Matching prefers
// the session's git_root (the repository root, which is what projectPath
// represents) and falls back to cwd for sessions that don't record
// git_root.
