...
//
// Newer Copilot CLI releases have been observed nesting session directories
// under an extra workspace-scoped directory (e.g.
// session-state/<workspace-id>/<session-id>/ instead of the older flat
// session-state/<session-id>/ layout). Rather than assume a fixed depth, we
// walk the whole tree looking for workspace.yaml files and pick whichever
// recent, project-matching one was modified most recently.
