...
// It prefers a session whose workspace.yaml cwd matches projectPath, but falls back to the
// most recently modified session directory within the timeout window when no metadata match
// is found (e.g. workspace.yaml is missing, unparseable, or its schema changed in a newer
// Copilot CLI release).
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	// the timeout window, regardless of whether its metadata matched projectPath.
	...
	for _, entry := range entries {
		...
		// The directory name is the session ID (see WriteSessionFile); prefer the
		// ID recorded in workspace.yaml when it's present and parses successfully.
