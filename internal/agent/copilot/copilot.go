...
// most recently modified session directory when metadata is missing or unparseable (e.g. the
// on-disk metadata format changed between Copilot CLI versions).
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// whether its metadata parses, in case the metadata format changed.
