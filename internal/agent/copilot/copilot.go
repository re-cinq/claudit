...
//
// Session metadata (workspace.yaml) is matched against projectPath by CWD when
// possible. Newer Copilot CLI releases have been observed to omit or rename
// metadata fields, which would otherwise cause every candidate to be silently
// filtered out. To stay resilient to that kind of drift, we also track the
// single most-recently-modified session in the timeout window as a fallback
// and use it when no CWD match is found — safe in practice because session
// discovery only ever runs against an isolated project checkout.
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// Directory name doubles as the session ID for Copilot; prefer the
		// parsed metadata ID when available.
