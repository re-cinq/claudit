...
//
// Session directories are preferred when their workspace.yaml cwd matches
// projectPath exactly. If no session's metadata can be matched (e.g. a
// Copilot CLI update renamed/dropped the cwd field, or workspace.yaml is
// otherwise unparseable), fall back to the most recently modified session
// directory within the recency window, mirroring the recency-only discovery
...
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// The directory name is the session ID (see WriteSessionFile); use it
		// as a fallback in case workspace.yaml's id field is missing/renamed.
