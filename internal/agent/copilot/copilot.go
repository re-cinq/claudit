...
// Sessions may be nested at varying depths depending on the Copilot CLI version
// (e.g. session-state/<id>/workspace.yaml vs session-state/<bucket>/<id>/workspace.yaml),
// so this walks the whole tree looking for workspace.yaml files instead of assuming
...
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	_ = filepath.WalkDir(sessionDir, func(path string, d fs.DirEntry, err error) error {
		if err != nil || d == nil || d.IsDir() || d.Name() != "workspace.yaml" {
			return nil
		}
