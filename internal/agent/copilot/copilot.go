...
// Sessions are located by walking the whole tree for workspace.yaml metadata files
// rather than assuming a fixed nesting depth, since Copilot CLI has changed how
// deeply session directories are nested under session-state/ across releases.
