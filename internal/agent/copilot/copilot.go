...
//
// Metadata (workspace.yaml) is preferred for matching a session to the
// working directory, but newer Copilot CLI releases have been observed to
// omit, rename, or restructure that file. To stay resilient to such upstream
// changes, if no session directory can be matched by CWD, we fall back to
// the most recently modified session directory that has a transcript file,
...
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// Track the newest recent session that has a transcript as a
		// fallback, in case metadata-based CWD matching below finds nothing.
