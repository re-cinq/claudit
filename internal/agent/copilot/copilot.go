...
// Newer Copilot CLI releases have been observed nesting session directories more
// deeply (e.g. under an account/workspace-hash directory) instead of storing
// workspace.yaml directly under session-state/<sessionID>/, so this walks the
// whole tree rather than assuming a single level of nesting. It also considers
// the transcript file's mtime (not just the session directory's), since some
// versions only touch events.jsonl as a session progresses without updating the
// parent directory's own modification time.
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	_ = filepath.WalkDir(sessionDir, func(p string, d os.DirEntry, walkErr error) error {
		if walkErr != nil || d == nil || d.IsDir() || d.Name() != "workspace.yaml" {
			return nil
		}
...
// directory's own metadata and its transcript file.
func latestModTime(dir string) time.Time {
	var best time.Time
	if info, err := os.Stat(dir); err == nil && info.ModTime().After(best) {
		best = info.ModTime()
	}
	if info, err := os.Stat(GetTranscriptPath(dir)); err == nil && info.ModTime().After(best) {
		best = info.ModTime()
	}
	return best
}
