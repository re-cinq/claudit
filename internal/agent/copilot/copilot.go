...
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// cwd with the exact process working directory used by the manual
		// post-commit hook. Fall back to matching against the recorded git
		// root, which is more stable across CLI versions.
