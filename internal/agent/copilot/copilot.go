...
//
// Sessions are primarily matched by comparing workspace.yaml's cwd field against
// projectPath. As a fallback (used when workspace.yaml is missing, unparseable, or
// its cwd no longer matches — e.g. after a Copilot CLI update changes how or when
// session metadata is written to disk), the most recently modified session
// directory within the timeout window is used instead. This mirrors the
// recency-only matching already used for Claude and Gemini sessions, and is safe
// here because discovery is scoped to a single project's post-commit hook
// invocation.
func scanForRecentSession(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		// whether its metadata is readable or matches, so we still have a
		// candidate if workspace.yaml is missing/renamed or its cwd field
		// doesn't line up (e.g. due to a Copilot CLI format change).
