...
// Newer OpenCode releases have been observed storing each session as its own
// subdirectory (e.g. <session-dir>/<session-id>/...) rather than a flat
// <session-id>.json file, so both layouts are considered here.
func (a *Agent) discoverFromFlatFiles(projectPath string) (*agent.SessionInfo, error) {
	...
	for _, entry := range dirEntries {
		name := entry.Name()
		isJSONFile := !entry.IsDir() && strings.HasSuffix(name, ".json")
		isSessionDir := entry.IsDir()
		if !isJSONFile && !isSessionDir {
			continue
		}
...
// candidateDBPaths returns possible locations for OpenCode's SQLite database,
// since the exact path and filename have moved across releases.
func candidateDBPaths(dataDir string) []string {
	return []string{
...
// findRecentSessionID queries dbPath for the most recent session, trying
// progressively looser queries in case the schema's column names or the
// project scoping no longer match what earlier OpenCode versions used.
...
// fetchSessionMessages queries dbPath for a session's messages as a JSON array,
// trying multiple column-name variants for resilience against schema changes.
...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	var dbPath string
	for _, candidate := range candidateDBPaths(dataDir) {
