...
//
// "tool.execute.before" also records a lightweight pointer to the active
// session (.shiftlog/active-session.json), written synchronously so it is
// never subject to the delay OpenCode's own session storage can have right
// after its CLI process exits. This lets a later *manual* `git commit` (one
// made outside the agent, so no "git commit" tool call ever happens) still
// be attributed to the session that was active moments before.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const recordActiveSession = async (sessionID) => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(shiftlogDir, "active-session.json"), JSON.stringify(pointer));
    } catch (e) {
      // Best-effort only - never disrupt the agent's workflow
    }
  };
