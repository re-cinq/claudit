...
//
// On every "after" event we also persist a pointer to the active session to
// .shiftlog/opencode-active-session.json in the project directory. OpenCode's
// own on-disk session storage layout has changed across releases (flat files,
// then SQLite, and beyond), so a manual (non-agent) git commit can't reliably
// discover the active session by reading OpenCode's internals. Writing our
// own bookkeeping file here — using only the stable plugin/SDK API — lets
// shiftlog find the session regardless of how OpenCode stores it internally.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Persists a pointer to the most recently active session so shiftlog can
  // find it later for a manual (non-agent) git commit, independent of
  // OpenCode's own internal storage layout.
  const persistActiveSession = async (sessionID) => {
    if (!sessionID) return;
    try {
      ...
      // Silently ignore - active session tracking is best-effort
    }
  };
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      await persistActiveSession(input.sessionID);
