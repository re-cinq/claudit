...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API. Bounded with a timeout
      // so a slow or unresponsive client can never block the tool call from
      // completing - we always fall back to the data_dir approach below.
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          const timeout = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("shiftlog: client.session.messages timed out")), 10000)
          );
