...
//
// OpenCode's on-disk session storage format has changed across releases and
// isn't a documented, versioned API, so we don't rely on it. Instead, on
// every tool call we fetch the session transcript via the plugin SDK client
// (which is stable) and record it to .shiftlog/opencode-active-session.json.
// This lets shiftlog's post-commit hook discover the active session for
// manual (non-agent) commits without needing to understand OpenCode's
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const fetchTranscript = async (sessionID) => {
    ...
    try {
      ...
      // Ignore - caller falls back to an empty transcript
    }
    return "";
  };
  ...
  const recordActiveSession = async (sessionID) => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(dir, "opencode-active-session.json"),
        JSON.stringify({
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Keep the active-session marker fresh on every tool call so manual
      // (non-agent) commits can still be attributed to this session.
