...
// tool execution hooks and calls shiftlog store after shell tool calls.
//
...
//
// We invoke shiftlog on every shell tool call, not just ones that look like a
// git commit: shiftlog needs to see non-commit calls too so it can record which
// session is active for this project (used to attribute a later *manual*
// `git commit`, which the agent process won't be around to help with).
// IsCommitCommand on the Go side still decides whether a given call actually
// gets stored as a conversation note.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  const shellTools = new Set(["bash", "shell", "terminal", "execute", "run", "command"]);
