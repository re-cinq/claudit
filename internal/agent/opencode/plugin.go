...
//
// shiftlog is spawned detached and asynchronously (not execSync) so that a
// slow or stalled `shiftlog store` invocation can never block OpenCode's
// tool-execution pipeline while we wait for it to finish.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Fire-and-forget: spawn shiftlog detached so a slow store never blocks
      // this hook (and therefore OpenCode's tool-execution pipeline).
      try {
