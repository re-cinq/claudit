...
//
// OpenCode's on-disk session storage layout is an internal implementation
// detail that has changed across releases, so shiftlog does not rely on it
// to discover sessions. Instead, on every tool call we snapshot the active
// session (via the SDK client) to a file inside the project's .opencode
// directory. This lets shiftlog's post-commit hook (which runs as a
// separate process with no access to the plugin's client) discover the most
// recent session for commits that happen outside of an agent tool call
// (e.g. a manual "git commit").
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Snapshot the session on every tool call (not just commits) so a
      // manual, out-of-band "git commit" can still be attributed to the
      // most recent session by shiftlog's post-commit hook, regardless of
      // how this version of OpenCode stores sessions on disk internally.
      if (transcriptData && input.sessionID) {
        try {
          const { mkdirSync, writeFileSync } = await import("fs");
          const { join } = await import("path");
          const snapshotDir = join(directory, ".opencode");
          mkdirSync(snapshotDir, { recursive: true });
          writeFileSync(join(snapshotDir, "shiftlog-session.json"), JSON.stringify({
