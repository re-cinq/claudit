...
//
// The "after" hook intentionally does not call back into the SDK client (e.g.
// client.session.messages): that re-enters the same OpenCode server that is
// still handling the in-flight tool-execution request, and on current
// OpenCode versions it never resolves, hanging the whole run. Instead,
// shiftlog store looks up the transcript itself (SQLite or flat-file
// storage) using the session_id and data_dir passed below.
