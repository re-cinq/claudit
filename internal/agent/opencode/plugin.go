...
//
// The SDK client's session.messages() call is bounded with a timeout: some
// OpenCode releases have changed this API's shape/behavior in ways that can
// leave the returned promise unresolved, which would otherwise hang the
// "after" hook (and thus the whole OpenCode run) indefinitely. On timeout or
// error we fall back to letting shiftlog read the transcript from disk via
// data_dir + session_id.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API, bounded by a timeout
      // so an unresponsive or changed client API can't hang this hook.
      let transcriptData = "";
