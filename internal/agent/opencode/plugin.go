...
//
// On every "after" call we also write a small marker file (.shiftlog/opencode-active-session.json)
// recording the current session ID and a fetched transcript snapshot via OpenCode's live
// SDK client. This lets shiftlog discover the active session for commits made *outside*
// of OpenCode's own tool execution (e.g. a manual `git commit` run by the user), without
// depending on OpenCode's internal on-disk session storage layout, which has changed
// across OpenCode versions and cannot be reliably reverse-engineered.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Fetch the session transcript via OpenCode's live SDK client. This is
  // preferred over reading OpenCode's on-disk session storage directly,
  // since that internal layout has changed across OpenCode versions.
  const fetchTranscript = async (sessionID) => {
    ...
    try {
      ...
      // Fall back to empty transcript below
    }
    return "";
  };

  // Record the active session to disk on every tool call so that manual
  // (non-agent) git commits can discover the session afterwards without
  // depending on OpenCode's internal storage layout.
  const updateActiveSessionMarker = async (sessionID, transcriptData) => {
    if (!sessionID) return;
    try {
      ...
      fs.writeFileSync(
        path.join(shiftlogDir, "opencode-active-session.json"),
        JSON.stringify({ session_id: sessionID, transcript_data: transcriptData })
      );
    } catch (e) {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      await updateActiveSessionMarker(input.sessionID, transcriptData);
