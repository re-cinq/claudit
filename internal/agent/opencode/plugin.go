...
//
// The "after" hook deliberately avoids calling back into OpenCode's own SDK
// client (e.g. client.session.messages) from within a tool-execution hook.
// That call re-enters the same OpenCode server process that is still
// synchronously awaiting this hook, and on some OpenCode versions the
// promise never resolves — hanging the entire CLI run indefinitely. Instead,
// we pass the on-disk data directory and session ID to `shiftlog store`,
// which reconstructs the transcript from OpenCode's session storage files.
