...
//
// Note: the plugin intentionally does not destructure/use the SDK "client"
// argument passed to the plugin factory. Requesting it causes OpenCode to
// eagerly establish a client connection for the plugin, which has been
// observed to make "opencode run" hang indefinitely on newer OpenCode
// releases (repro: hangs even when no git-commit tool call ever occurs,
// i.e. before any of our commit-handling logic runs). Transcript data is
// instead reconstructed on the Go side from data_dir + session_id (see
// ParseHookInput), with the post-commit git hook's DiscoverSession
// providing a storage-format-aware (flat file or SQLite) fallback.
