...
//
// The SDK client call in the "after" hook is wrapped with a timeout: newer
// OpenCode releases have changed/renamed session client methods, and an
// unresolved promise there would otherwise block the "after" hook (and thus
// OpenCode's own tool-execution loop) indefinitely.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API. Guarded with a timeout
      // so a renamed/incompatible client method can't hang the hook forever.
      let transcriptData = "";
