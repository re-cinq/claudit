...
//
// On every "after" event we also refresh a local active-session marker via
// `shiftlog session-start` (writing the transcript to .shiftlog/ inside the
// project). This lets a manual `git commit` made outside the agent (i.e. not
// via a tool call) still be attributed to the right session, without shiftlog
// needing to know OpenCode's internal on-disk storage format — that format
// has changed across OpenCode versions and isn't a stable integration point.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Refresh the .shiftlog/active-session.json marker so shiftlog can find
  // this session later even if the eventual commit isn't made by the agent.
  const touchActiveSession = async (sessionID) => {
    if (!sessionID) return;
    try {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Keep the active-session marker fresh on every tool call so a
      // manual commit made after this turn can still be attributed.
      await touchActiveSession(input.sessionID);
