...
//
// Deliberately does NOT destructure/use the plugin factory's SDK `client`
// handle: awaiting `client.session.*` calls from within a tool-execution
// hook has been observed to block indefinitely against newer OpenCode CLI
// releases, hanging the whole "opencode run" session. The data_dir/session_id
// reconstruction below is sufficient for shiftlog to locate the transcript
// after the fact, so no SDK round-trip is needed here.
