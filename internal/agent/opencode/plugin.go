...
//
// The "after" hook must never block the host process: newer OpenCode releases
// appear to run plugin hooks on the same event loop that drives the agent's
// tool-call loop, so a synchronous, blocking call here (e.g. execSync) can
// stall the entire session for as long as the subprocess/API call takes. We
// therefore fire the shiftlog store subprocess without awaiting it, and cap
// the SDK client call with an explicit timeout so a slow/hung API can never
// wedge the hook indefinitely.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API. Bounded with a
      // timeout so an API/version mismatch can't hang this hook forever.
      let transcriptData = "";
      ...
      // Fire-and-forget: never block the plugin hook (and therefore the
      // agent's tool-call loop) on the shiftlog store subprocess.
      try {
