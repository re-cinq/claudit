...
//
// On every "after" call we also refresh a local session cache
// (.shiftlog/opencode-session.json) via the SDK client. OpenCode's internal
// on-disk session storage format is not a stable integration point and has
// changed across releases, so a later *manual* commit (made outside the
// agent, after this process has exited) reads that cache via
// `shiftlog store --manual` instead of trying to reverse-engineer OpenCode's
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  let lastTranscript = null; // { sessionID, transcriptData }
  ...
  // Keeps .shiftlog/opencode-session.json up to date with the active
  // session's transcript so manual (non-agent) commits can still be
  // annotated later, without depending on OpenCode's own storage format.
  const cacheSession = async (sessionID) => {
    const transcriptData = await fetchTranscript(sessionID);
    if (!transcriptData) return;
    lastTranscript = { sessionID, transcriptData };

    try {
      const { mkdirSync, writeFileSync } = await import("fs");
      const { join } = await import("path");
      const cacheDir = join(directory, ".shiftlog");
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Reuse the transcript we just cached above when possible, to avoid
      // fetching it twice from the SDK client.
      const transcriptData =
        lastTranscript && lastTranscript.sessionID === pending.sessionID
          ? lastTranscript.transcriptData
          : await fetchTranscript(pending.sessionID);
