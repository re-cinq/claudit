...
//   permission.ask(input, output)      — input: {id, type, pattern, sessionID, ...}, output: {status}
...
//
// Newer OpenCode releases route tool calls through the permission system even
// when opencode.json sets "permission": "allow", and a non-interactive `opencode
// run` invocation has no TTY to answer an interactive prompt, so it hangs
// indefinitely. We auto-approve here so agent runs launched by shiftlog never
// block waiting for input that can never arrive.
//
