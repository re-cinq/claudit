...
//
// On every "after" hook we also cache the session (via the SDK's
// client.session.messages() call) to a file under .git/. OpenCode's internal
// session storage format (flat files vs SQLite, exact schema) has changed
// across releases and is not something we can reliably reverse-engineer, but
// the plugin SDK is a stable, documented public API. Caching through it lets
// shiftlog's manual (post-commit hook) session discovery work for commits
// made outside of an agent tool call, without needing to know where or how
// OpenCode stores sessions on disk.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (pending) pendingCommits.delete(input.callID);
      ...
      if (client && input.sessionID) {
        try {
          ...
          // Fall back to whatever we have (possibly empty)
        }
      }

      // Cache the session for manual (post-commit hook) discovery. Best
      // effort: never let a cache write failure disrupt the plugin.
      if (input.sessionID) {
        try {
          ...
          if (fs.existsSync(gitDir)) {
            fs.writeFileSync(path.join(gitDir, "shiftlog-opencode-session.json"), JSON.stringify({
