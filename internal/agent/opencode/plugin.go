...
// tool execution hooks and calls shiftlog store after every shell command.
//
...
//
// We invoke "shiftlog store" for every bash call, not just ones containing a
// git commit — the same way the other agents' hooks work. shiftlog's own
// IsCommitCommand check decides whether to actually write a note. Firing on
// every call (rather than pre-filtering here) also lets shiftlog snapshot the
// live session as it goes, which is what makes a later manual "git commit"
// (made after this session's process has already exited, with nothing else
// left for shiftlog to discover on disk) still attributable to this session.
