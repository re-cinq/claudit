...
//
// OpenCode's on-disk session storage format (flat files vs. SQLite, directory
// layout, project ID scheme, ...) has changed across releases and is not a
// stable contract to depend on. To avoid reverse-engineering those internals,
// we snapshot the active session via the plugin SDK client (client.session.messages),
// which is OpenCode's supported API surface, into .shiftlog/opencode-session-cache.json
// on every tool call. This lets DiscoverSession (used for manual, non-agent
// commits) find the right session without relying on storage internals.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const cacheActiveSession = async (sessionID) => {
    ...
    try {
      ...
      writeFileSync(
        directory + "/.shiftlog/opencode-session-cache.json",
        JSON.stringify({
      ...
      // Best-effort cache; never disrupt the agent session on failure.
    }
  };
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      // Keep a fresh snapshot of the active session so manual (non-agent)
      // commits can still be attributed via the post-commit hook.
      await cacheActiveSession(input.sessionID);
    },
